...
SELECT
    ...
    purchases,
    ROUND(
        100.0 * purchases / SUM(purchases) OVER (),
        2
    ) AS purchase_share_pct,
    ROUND(
        100.0 * impressions / SUM(impressions) OVER (),
        2
    ) AS impression_share_pct
FROM cte
GROUP BY ad_type
...
SELECT 
    ...
    ROUND((p.total_budget * 1000) / c.impressions) as cpm, 
    ROUND(p.total_budget / c.clicks) as cpc
FROM cte c
LEFT JOIN campaigns p
...
SELECT
    a.id,
    ROUND(c.total_budget/a.purchase,2)as cpa,
    ROUND(c.total_budget/a.clicks,2) as cpc,
    ROUND(SUM(c.total_budget) OVER ()/ SUM(a.purchase) OVER (),2) as avg_cpa,
    ROUND(SUM(c.total_budget) OVER ()/ SUM(a.clicks) OVER (),2) as avg_cpc
FROM ad_purchase a
LEFT JOIN campaigns c ON c.campaign_id = a.id
ORDER BY cpa ASC
LIMIT 10;
...
SELECT
    ...
    MAX(CASE WHEN ctr_rank = 1 THEN campaign_id END) AS top_ctr_campaign,
    MAX(CASE WHEN ctr_rank = 1 THEN ctr END) AS top_ctr,

    MAX(CASE WHEN cvr_rank = 1 THEN campaign_id END) AS top_cvr_campaign,
    MAX(CASE WHEN cvr_rank = 1 THEN conversion END) AS top_cvr
...
SELECT
    ...
    c.user_id,
    c.duration,
    ROW_NUMBER() OVER(PARTITION BY g.gender ORDER BY duration ASC) AS rn,
    COUNT(*) OVER (PARTITION BY g.gender) AS total
FROM conversion_time c
LEFT JOIN gender g ON c.user_id = g.user_id
)
SELECT
    gender,    
